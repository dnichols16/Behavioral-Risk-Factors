...
It's important at this point to remove the data that you have downloaded (these are BIG files), or your RAM will overload.
```{r, message = FALSE}
rm(brf17)
rm(brf17a)
rm(brf16)
...
Now we have a stable mechanism to get the rest of the matching columns from each year. I went all way back to 1989 to see what variables I could follow through the whole 30 year period. When the survey started, they didn't ask many questions, but there are a few that you can follow through the whole period. I could have written this recurring code as a function but felt that I had more control doing each year separately. 
```{r, message = FALSE}
brf15 <- readRDS("brf15")
brf15$year = "2015"
...
I've hidden the bulk of the calculations, but here is the last one. They are really all just rinse and repeat.
```{r, message = FALSE}
brf89 <- readRDS("brf89")  
brf89$year = "1989"
...
Let's ask the question: Has the proportion of smokers to non-smokers changed over time? We have 30 years of self-reported data answering the BFRSS survey question "Have you smoked at least 100 cigarettes in your entire life?". We can assume that this question will give us some insight into the answer to our research question. smok is our compilation of BFRSS data from 1989 to 2017.

```{r, message = FALSE}
glimpse(brf_all_from89)
smok <- brf_all_from89 
```
...
colnames(smok) <- tolower(colnames(smok)) #less typing
#filter out only observations answering the smoking question
smok1 <- smok %>%
  filter(smoke100 == 2 | smoke100 == 1)
#get a count of smokers in each year of the survey
smokers <- smok1 %>% 
  group_by(year) %>% 
  count(smoke100)
#get the data in a better form for plotting
...
ggplot(smokers_wide) +
  geom_line(aes(x = as.numeric(year), y = smok_prop), color = "blue") +
  labs(x = "Year", y = "Smokers/Non-Smokers",
    title = "Proportion of Smokers to Non-Smokers",
    subtitle = "Have you smoked at least 100 cigarettes in your entire life?",
    caption = "Behavioral Risk Factor Surveillance System")
  
```

There is a definite trend over the last 30 years to decrease the proportion of smokers to non-smokers.<br>

We can ask other questions relating to other health subjects from the BRFSS surveys. This time we'll look at data from 2007 through 2017 asking questions about diabetes. I will get the data from these years and again will hide the coding to save space on the blog post.
